...
# RSEARCH V1.0 : ROFI FILES SEARCH TOOL
...
dtype=""
dtype2="%y  "
#default mode indicator (0=update, 1=normal: sorted home files, 2=sorted home files including hidden files, 3=whole filesystem, without any sorting)
mode=1
...
rsearch generates a "database" file listing your files from the initial path of your choice. Your home is the default initial path. It sorts them by access times, displays modification times (the access time is used for sorting but not displayed), and lets you search through your files thanks to rofi -dmenu.
...
You can use it with "-i 0" option to generate the database each time the script is called. If the database takes too much time to be generated when you call the script, you can separate database generation (at startup, and/or when you call the script but only if the database is too old, and/or every x min/h with a cron task) and database searching. Thus your searching is snappier and easier. Default behaviour is updating if database is older than 3 hours.

rsearch generates a database file for each initial path. Thus you can use it to search in different folders or media, each with its own database file making rofi interface to snap on your screen.

With default settings only your home files are listed, and the hidden files are... hidden. You can include hidden files with -hf option (except for cache folders), or even search though you whole file system. In this last mode the script use locate command instead of find, to be quick enough, and files are not sorted by access time.
...
YOU MAY USE FOLLOWING OPTIONS :
	...
	-hf (or --hidden-files) : home files including hidden files. Only cache folder are hidden. File are sorted by access time.
	-a (or -all) : global search : initial path is / and hidden files are included. So you can access all files on you system files. This mode uses loocate commande, which comes with its database management. Files are not sorted.
	...
	-p (or --path) initial-path : default initial path to search from (default : ~, which is your home). With this option, files are displayed with their short path, without initial path, which is easier to read.
	-mp (or --multiple-paths) "path1 path2 path3" : several initial paths to search from. With this option, files are displayed with their full path, including initial path. May work with only one path of course. Paths should be quoted together.
	-l (or --lines) number-of-diplayed-files : modify number of displayed files in rofi -dmenu (default is 20). One file by line.
	-w (or --width) rofi-window-width : modify rofi window width. It is a rofi option, under 100 it is a percentage of your screen, above 100 it is a number of pixels. Default is 60 (which is 60% of your screen width)
...
TO USE THIS SCRIPT :
	...
	2. Add a shortcut to call the script easily. In fluxbox for example, add in your keys file such a line :
		...
		Shift F2 :Exec ~/.local/bin/rsearch -hf - i 240 -d -l 20 (include hidden files, update if older than 4 hour, display 20 lines and include folders)
...
In addition you may :
	...
	3.b Adapt the interval between two updates. Default behaviour is following : when the script is called it checks how old is the database file. If older than 3 hours it updates the database, else it use the existing one. You can modify this interval with -i option. I you use -i 0, the script updates the database each time it is called, and no database file is kept.
...
' | less
	...
	elif [[ $1 = "-u" || $1 = "--update" ]] ; then mode=0 ; shift
...
function openfile () {
        ...
        file_type=$(echo "$1"|awk -F '.' '{printf $NF}')

        if [ "$file_type" == "jpg" ] || [ "$file_type" == "jpeg" ] || [ "$file_type" == "png" ]; then
            filemgr=$picmgr
		...
		$filemgr "$1" 2>/dev/null &
}

### mode 3 : whole filesystem with locate command, without any sorting
#########################################################################
if [[ $mode == 3 ]] ; then
	choice=$(locate / | rofi -dmenu -i -width $width -columns $columns -l $lines -p Rsearch -keep-right -no-custom )
	#dmenu version
...
### mode 0, 1 and 2 : update check, and update if needed (if given interval is over)
#########################################################################
if [[ $mode -ge 1 ]] && [[ -e $dbpath ]] && [[ $(date -r $dbpath +%s) -gt $(($(date +%s)-$interval*60)) ]] ; then
	...
	if [[ $mode == 0 ]] ; then echo "database $dbpath updated, end of script" ; exit 0 ; fi
...
#########################################################################
choice=$( rofi -dmenu -input $dbpath -i -width $width -columns $columns -l $lines -p "Rsearch" -keep-right -no-custom )
#dmenu version
...
if [[ $choice != "" ]] ; then $(openfile "$fullname") ; fi
